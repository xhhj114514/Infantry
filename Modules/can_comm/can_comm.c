 ...
 * @param ins 需要重置的实例
 */
static void BoardCommResetRx(BoardCommInstance *ins)
{
    // 当前已经收到的buffer清零
 ...
 * @param _instance
 */
static void BoardCommRxCallback(CANInstance *_instance)
{
    BoardCommInstance *comm = (BoardCommInstance *)_instance->id; // 注意写法,将can instance的id强制转换为BoardCommInstance(CANCommInstance*)类型
...
static void BoardCommLostCallback(void *cancomm)
{
    BoardCommInstance *comm = (BoardCommInstance *)cancomm;
    BoardCommResetRx(comm);
}

BoardCommInstance *BoardCommInit(BoardComm_Init_Config_s *comm_config)
{
    BoardCommInstance *ins = (BoardCommInstance *)malloc(sizeof(BoardCommInstance));
    memset(ins, 0, sizeof(BoardCommInstance));
    ...
    comm_config->can_config.can_module_callback = BoardCommRxCallback;
...
void BoardCommSend(BoardCommInstance *instance, uint8_t *data)
{
    static uint8_t crc8;
...
void *BoardCommGet(BoardCommInstance *instance)
{
    instance->update_flag = 0; // 读取后将更新flag置为0
    return instance->unpacked_recv_data;
}

uint8_t BoardCommIsOnline(BoardCommInstance *instance)
{
